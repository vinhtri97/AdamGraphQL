...
import { updateDocument, getCollection } from "../../../MongooseFunctions";
...
export class TournamentMutationService {
    ...
    async addTeam(
        ...
        if (`${ageGroup}` in tournament.age_groups) {
            if (
                !tournament.age_groups[`${ageGroup}`].teams.includes(
                    ObjectId(teamID)
                )
            )
                tournament.age_groups[`${ageGroup}`].teams.push(
                    ObjectId(teamID)
                );
    ...
    async removeTeam(
        ...
        const tournament: any = await getCollection(Tournament, tournamentID);
        if (`${ageGroup}` in tournament.age_groups) {
            tournament.age_groups[`${ageGroup}`].teams = tournament.age_groups[
                `${ageGroup}`
            ].teams.filter(
                (nestedTeamID: { toString: () => string }) =>
                    nestedTeamID.toString() !== teamID
            );
            await Team.findByIdAndUpdate(teamID, {
