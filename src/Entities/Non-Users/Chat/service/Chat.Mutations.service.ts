...
export class ChatMutationService {
    async createChat(input: CreateChatInput): Promise<boolean | Error> {
        ...
        if ('team_id' in input) {
            ...
            if (!mongoose.Types.ObjectId.isValid(team_id)) throw new Error(`Invalid Team ID: ${team_id}`);
    ...
    async addUser(chatID: string, userType: string, userID: string): Promise<boolean | Error> {
        ...
        } else throw new Error("The user's type must be 'Player' or 'Coach' or 'Spectator' or 'Director'.");
        if (foundChat && foundUser) {
            const alreadyInChat = foundChat.users.some(({ id }: { [key: string]: any }) => id.toString() == userID);
            const alreadyHasChat = foundUser.chats.some(
                ({ chat_id }: { [key: string]: any }) => chat_id.toString() == chatID
            );
    ...
    async removeUser(chatID: string, userType: string, userID: string): Promise<boolean | Error> {
        const foundChat: any = await Chat.findById(chatID).limit(1);
        let foundUser: any = false;
        if (userType == 'Player') {
            foundUser = await Player.findById(userID).limit(1);
        ...
        if (foundChat && foundUser) {
            foundChat.users = foundChat.users.filter(({ id }: { [key: string]: any }) => id.toString() != userID);
    ...
    async changeMutedStatus(
        ...
        const foundChat: any = await Chat.findById(chatID).limit(1);
        if (foundChat) {
            const foundIndex = foundChat.users.findIndex(({ id }: { [key: string]: any }) => id.toString() == userID);
            if (foundIndex > -1) {
