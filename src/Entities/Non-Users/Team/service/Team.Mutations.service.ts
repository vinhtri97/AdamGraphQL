...
import { addToObjArray, addToStringArray, removeFromStringArray, updateDocument } from '../../../../MongooseFunctions';
...
export class TeamMutationService {
    async createTeam(input: CreateTeamInput): Promise<boolean | Error> {
        const { coaches } = input;
        if (coaches.length < 1) throw new Error('Invalid coaches array: There must be at least 1 coach.');
        ...
        if (foundCoaches.length > 0 && foundCoaches.length == coaches.length) {
            ...
            createdTeam.save();
            return true;
        } else throw new Error('Cannot create team because a coachID was invalid');
    }
    ...
    async addPlayer(teamID: string, playerID: string): Promise<boolean | Error> {
        // TODO change auto-accept
        const res = await addToObjArray(Team, 'players', teamID, { accepted: false }, Player, 'teams', playerID, {
            accepted: false
        });
        return res;
    }
    ...
    async addCoach(teamID: string, coachID: string): Promise<boolean | Error> {
        return await addToStringArray(Team, 'coaches', teamID, Coach, 'teams', coachID);
    }

    async removePlayer(teamID: string, playerID: string): Promise<boolean | Error> {
        return await handleTeamChatsForParents(teamID, playerID, false);
    }

    async removeCoach(teamID: string, coachID: string): Promise<boolean | Error> {
        return await removeFromStringArray(Team, 'coaches', teamID, Coach, 'teams', coachID);
    }
}
